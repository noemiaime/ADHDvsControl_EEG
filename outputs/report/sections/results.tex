...
Le feature EEG sono state confrontate tra soggetti ADHD e soggetti Control mediante test di Mann-Whitney U. Per quantificare la grandezza della differenza tra i due gruppi è stato calcolato Cohen's $d$. Le prime dieci feature ordinate secondo il valore assoluto di Cohen's $d$ sono riportate nella Tabella~\ref{tab:top10_cohen}.
...
Le feature con maggiore effect size sono prevalentemente associate al rapporto theta/beta e alle potenze nelle bande lente, soprattutto in regioni temporali, parietali e occipitali. Tutte le prime dieci feature mostrano Cohen's $d$ negativo, indicando valori medi più elevati nel gruppo Control. Questo suggerisce che, nel dataset analizzato, la separazione tra ADHD e Control sia legata soprattutto a differenze nelle componenti spettrali lente e nei rapporti tra bande.
...
La matrice regionale mostra effect size prevalentemente negativi, in particolare per la banda delta e per il rapporto theta/beta. Il valore più marcato riguarda il rapporto theta/beta temporale, che risulta anche significativo dopo correzione FDR. Questo risultato è coerente con la Tabella~\ref{tab:top10_cohen}, dove la stessa feature presenta il maggiore valore assoluto di Cohen's $d$.
...
La Figura~\ref{fig:fdr_features_boxplot} mostra che alcune feature significative presentano distribuzioni con forte variabilità inter-soggetto e presenza di valori estremi, soprattutto nelle potenze di banda. Questo conferma l'utilità di un approccio non parametrico per il confronto statistico tra gruppi.
...
Le feature regionali ottengono la migliore balanced accuracy media e il miglior ROC-AUC, pur utilizzando solo 25 variabili. Le differenze rispetto all'insieme completo non sono ampie, ma il risultato suggerisce che una rappresentazione regionale sia sufficiente a conservare informazione discriminativa, riducendo la dimensionalità rispetto alle feature relative ai canali singoli.

\subsection{Performance del modello finale}

Il modello finale è stato addestrato sull'intero training/validation set utilizzando il miglior feature set, cioè le feature regionali, ed è stato valutato sul test set indipendente. Le metriche finali sono riportate nella Tabella~\ref{tab:rf_final_metrics}.

...
\subsection{Riduzione del feature set in base alla correlazione}

La matrice di correlazione tra le dieci feature più importanti ha mostrato la presenza di correlazioni moderate o elevate tra alcune variabili. Per questo motivo è stata effettuata un'analisi aggiuntiva per valutare se fosse possibile ridurre la ridondanza del feature set senza peggiorare le performance di classificazione.
...
La soglia selezionata è stata pari a 0.80, poiché ha ottenuto la migliore balanced accuracy media in cross-validation. Il feature set ridotto risultante contiene 19 feature. Dopo la selezione della soglia, il modello Random Forest è stato nuovamente ottimizzato sul feature set ridotto mediante la stessa procedura di tuning utilizzata per il modello principale.
...
Il confronto con il modello basato sul feature set originale permette di valutare se la rimozione delle feature ridondanti riduca, mantenga o migliori la capacità discriminativa del classificatore. Se le performance rimangono simili, il risultato suggerisce che parte dell'informazione contenuta nelle feature originali era ridondante; se invece peggiorano, significa che le feature rimosse contribuivano comunque alla classificazione.

\subsection{Studio feature theta/beta ratio}

L'analisi delle feature importance e degli effect size ha mostrato che alcune feature basate sul rapporto theta/beta sono tra le variabili più rilevanti del dataset, in particolare il rapporto theta/beta temporale. Questo risultato è coerente con la letteratura precedente, che ha spesso associato l'ADHD ad alterazioni del rapporto theta/beta. Tuttavia, alla luce della meta-analisi di Arns et al., è importante distinguere tra la presenza di informazione discriminativa nel TBR e la sua sufficienza come marker diagnostico autonomo \cite{Arns2013}.

Per questo motivo, è stata condotta un'analisi specifica utilizzando esclusivamente feature derivate dal rapporto theta/beta. Sono stati confrontati tre insiemi di feature: rapporti theta/beta regionali, rapporti theta/beta a livello di singolo canale e l'insieme completo di tutte le feature theta/beta ratio. L'obiettivo era verificare se l'informazione contenuta nel TBR fosse sufficiente per classificare ADHD e controlli, oppure se il modello multifeature mantenesse un vantaggio predittivo.

Il confronto iniziale è stato effettuato mediante cross-validation stratificata a 5 fold sul training/validation set.
...
Il miglior insieme di feature theta/beta ratio è risultato essere \textbf{Channel-level TBR}. Questo insieme è stato quindi utilizzato per addestrare un modello Random Forest, ottimizzato con la stessa procedura adottata per i modelli precedenti.
...
Questa analisi permette di confrontare direttamente l'informazione contenuta nel rapporto theta/beta con quella contenuta nel modello multifeature. Se il modello basato solo su feature theta/beta ottiene performance inferiori rispetto al modello completo o al modello ridotto, ciò suggerisce che il rapporto theta/beta contiene informazione discriminativa ma non è sufficiente, da solo, per una classificazione robusta. Se invece le performance risultano simili, il risultato suggerisce che una parte rilevante dell'informazione discriminativa è catturata proprio dalle feature theta/beta ratio.
